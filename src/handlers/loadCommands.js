...
module.exports = async (client) => {
  ...
  await chillout.forEach(interactionFiles, (interactionFile) => {
    ...
    if (command.name.length > 3) {
      console.log(
        `[WARN] "${rltPath}" The name list of the interaction file is too long. (>3) Skipping..`
      );
      return;
    }
    ...
    if (!command.name?.length) {
      console.log(
        `[WARN] "${rltPath}" The interaction file does not have a name. Skipping..`
      );
      return;
    }
    ...
    if (client.slash.has(command.name)) {
      console.log(
        `[WARN] "${command.name[1]}" interaction has already been installed. It's skipping.`
      );
      return;
    }
