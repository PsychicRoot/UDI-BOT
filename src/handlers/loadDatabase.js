...
module.exports = async (client) => {
  ...
  client.createEVoice = async function (interaction) {
    const db = new Database("src/settings/models/voice.json", {
      databaseInObject: true,
    });
    await db.set(interaction.guild.id, {
      voice_enable: true,
  ...
  client.createPlay = async function (interaction, message) {
    const db = new Database("src/settings/models/message.json", {
      databaseInObject: true,
    });
    await db.set(interaction.guild.id, {
  ...
  client.addCount = async function (cmdname) {
    const db = new Database("src/settings/models/stats.json", {
      databaseInObject: true,
    });
    await db.add(cmdname, 1);
  };

  client.addChart = async function (id) {
    const db = new Database("src/settings/models/chart.json", {
      databaseInObject: true,
    });
    await db.add(id, 1);
  };
